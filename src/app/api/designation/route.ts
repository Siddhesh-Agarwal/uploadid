import db from "@/db";
import { designationTable } from "@/db/schema";
...
export async function GET(request: NextRequest) {
  ...
  const data = await db
    ...
    .where(eq(designationTable.instituteID, instituteID));
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await db.insert(designationTable).values(body).returning();
    return NextResponse.json(data);
  } catch {
    return NextResponse.json(
